...
def ventana_inicio():
    ...
    ventana_principal.title("Login Demo")#TITULO DE LA VENTANA
    global frame_login
    frame_login=Frame()
    frame_login.pack()
    frame_login.config(bg=pestas_color, width="650", height="350")
    Label(frame_login,text="Escoja su opción", bg="LightGreen", width="300", height="2", font=("Calibri", 13)).pack()#ETIQUETA CON TEXTO
    Label(frame_login,text="").pack()
    Button(frame_login,text="Acceder", height="2", width="30", bg=pestas_color, command=login).pack() #BOTÓN "Acceder"
    Label(frame_login,text="").pack()
    Button(frame_login,text="Registrarse", height="2", width="30", bg=pestas_color, command=registro).pack() #BOTÓN "Registrarse".
    Label(frame_login,text="").pack()
...
#CREAMOS evento para cerrar widget
def hide_me(event):
